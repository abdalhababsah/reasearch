...
export default function ResearchEdit({
    ...
    // Ensure files is always an array at runtime
    const researchFiles: ResearchFile[] = Array.isArray(research.files)
        ? research.files
        : [];
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        existing_file_visibility: Object.fromEntries(
            (researchFiles || []).map((f) => [f.id, Boolean(f.is_visible)])
    ...
    const [wallpaperPreview, setWallpaperPreview] = useState<string | null>(
        research.wallpaper_url
    );

    // Find current primary file from existing files (if needed later)
    const currentPrimaryFileId =
        researchFiles.find((f) => f.is_primary)?.id ?? null;
    ...
    const removeNewFileAt = (index: number) => {
        ...
        const nextVisibility = (data.file_visibility ?? []).filter(
            (_, i) => i !== index
        );
        setData('files', next as any);
    ...
    const removeExistingFile = (fileId: number) => {
        ...
        const visibility = { ...(data.existing_file_visibility || {}) };
        delete visibility[fileId];
        setData('existing_file_visibility', visibility as any);
    };

    const toggleExistingFileVisibility = (fileId: number, visible: boolean) => {
        const current = { ...(data.existing_file_visibility || {}) };
        current[fileId] = visible;
        setData('existing_file_visibility', current as any);
    };
    ...
    // Get existing files that haven't been marked for removal (safe)
    const activeExistingFiles: ResearchFile[] = (researchFiles || []).filter(
        (f) => f && !data.remove_file_ids.includes(f.id)
    );
    ...
    // Sanitize new files – filter out any accidental `undefined`
    const newFiles: File[] = Array.isArray(data.files)
        ? (data.files.filter(Boolean) as File[])
        : [];
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                {
                    title: research.title ?? t('researches.card.untitled'),
                    href: `/researcher/researches/${research.id}`,
                },
                { title: t('actions.edit'), href: '#' },
            ]}
        >
            <Head
                title={`${t('actions.edit')} • ${research.title ?? t('researches.card.untitled')
                    }`}
            />
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            {t('researches.form.editSubtitle', {
                                defaultValue:
                                    'Update your research details, manage files, and adjust settings.',
                            })}
                        </p>
                    </div>
                    ...
                    <div className="flex gap-2">
                        <Button
                            ...
                            variant="outline"
                            onClick={() =>
                                router.visit(`/researcher/researches/${research.id}`)
                            }
                        >
                            {t('actions.cancel')}
                        </Button>
                                        ...
                                        {activeExistingFiles.map((file) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <Checkbox
                                                        checked={
                                                            data.existing_file_visibility?.[file.id] ??
                                                            file.is_visible ??
                                                            true
                                                        }
                                                        onCheckedChange={(checked) =>
                                                            toggleExistingFileVisibility(
                                                                file.id,
                                                                Boolean(checked)
                                                            )
                                                        }
                            ...
                            <div className="rounded-lg border border-dashed bg-muted/40 p-4 text-center">
                                ...
                                <p className="mt-1 text-xs text-muted-foreground">
                                    {t('researches.form.dropFilesSubtitle', {
                                        defaultValue:
                                            'Any file type: docs, code, archives, datasets, etc.',
                                    })}
                                </p>
                                        ...
                                        {newFiles.map((file, index) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-1 flex-col gap-0.5">
                                                    <div className="flex items-center gap-2">
                                                        <span className="truncate max-w-[220px]">
                                                            {file.name ?? t('researches.form.unnamedFile', {
                            ...
                            <div className="mt-4 border-t pt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_200px] items-start">
                                <div className="space-y-2">
                                    ...
                                    {wallpaperPreview && !data.remove_wallpaper && (
                                        ...
                                        >
                                            {t('researches.form.removeWallpaper', {
                                                defaultValue: 'Remove Wallpaper',
                                            })}
                                        </Button>
                                    )}
                                    ...
                                    <div className="relative h-32 w-40 overflow-hidden rounded-lg border bg-muted">
                                        {wallpaperPreview && !data.remove_wallpaper ? (
                                            ...
                                            <div className="flex h-full w-full items-center justify-center text-xs text-muted-foreground">
                                                {t('researches.form.noWallpaperPreview', {
