...
export default function ResearchShow() {
  ...
  const getFileIcon = (mimeType: string, fileName: string) => {
    ...
    if (mimeType.includes('spreadsheet') || ['xls', 'xlsx', 'csv'].includes(ext || '')) return FileText;
    if (mimeType.includes('code') || ['js', 'ts', 'jsx', 'tsx', 'py', 'java'].includes(ext || '')) return FileText;
          ...
          <div className="container relative z-10 mx-auto px-4 py-8 max-w-7xl">
            ...
            >
              ...
              <span className="text-sm font-semibold text-primary">
                {t('research.publication', { defaultValue: 'Research Publication' })}
              </span>
            </motion.div>
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  >
                    {research.author.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                  </Link>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <a href={`mailto:${research.author.email}`}
                  ...
                  >
                    ...
                    {t('actions.contact', { defaultValue: 'Contact Author' })}
                  </a>
              ...
              {research.tags.length > 0 && (
                ...
                >
                  <h3 className="mb-4 text-xl font-bold flex items-center gap-2">
                    <TagIcon className="h-5 w-5 text-primary" />
                    {t('research.tags', { defaultValue: 'Research Tags' })}
                  </h3>
              ...
              >
                <div className="relative h-64">
                  ...
                  <div className="absolute top-4 right-4 rounded-2xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-md p-4 shadow-2xl">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-primary">{displayYear}</div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="pt-4 border-t-2 border-primary/10 grid grid-cols-2 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary">{research.files.length}</div>
                      <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">
                        {t('research.files', { defaultValue: 'Files' })}
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary">{research.categories.length}</div>
                      <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">
                        {t('research.categories', { defaultValue: 'Categories' })}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h2 className="text-4xl font-bold">
                      {t('research.filesAndDocuments', { defaultValue: 'Files & Documents' })}
                    </h2>
                    <p className="text-muted-foreground mt-1">
                      {research.files.length} {t('research.filesAvailable', { defaultValue: 'files available for download' })}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {hasFiles ? (
                <div className="rounded-3xl border-2 border-primary/10 bg-white dark:bg-gray-900 shadow-2xl overflow-hidden">
                  <div className="grid lg:grid-cols-[420px,1fr] min-h-[700px]">
                    {/* Left Side - File Explorer */}
                    <div className="border-r-2 border-primary/10 flex flex-col bg-gradient-to-b from-muted/20 to-background">
                      {/* Explorer Header */}
                      <div className="border-b-2 border-primary/10 bg-gradient-to-r from-primary/5 to-background p-5">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-lg">
                              {t('research.documents', { defaultValue: 'Documents' })}
                            </span>
                          </div>
                          <span className="text-sm font-semibold text-muted-foreground px-3 py-1 rounded-full bg-primary/10">
                            {filteredFiles.length} {t('research.items', { defaultValue: 'items' })}
                          </span>
                        </div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            type="text"
                            placeholder={t('research.searchFiles', { defaultValue: 'Search files...' })}
                            value={searchQuery}
                              ...
                              return (
                                ...
                                >
                                  <div className={`flex h-12 w-12 items-center justify-center rounded-xl flex-shrink-0 ${
                                    ...
                                    <FileIcon className={`h-6 w-6 ${isSelected ? 'text-primary' : 'text-primary/70'}`} />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                                      ...
                                      <span>•</span>
                                      <span className="truncate uppercase">{file.mime_type.split('/')[1]}</span>
                                    </div>
                                  </div>
                                  {file.is_primary && (
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {filteredFiles.map((file, i) => {
                              ...
                              return (
                                ...
                                >
                                  <div className={`mx-auto mb-3 flex h-20 w-20 items-center justify-center rounded-xl ${
                                    isSelected ? 'bg-primary/20' : 'bg-primary/10'
                                  }`}>
                                    <FileIcon className={`h-10 w-10 ${isSelected ? 'text-primary' : 'text-primary/70'}`} />
                                  </div>
                      ...
                      <div className="border-t-2 border-primary/10 bg-gradient-to-r from-primary/5 to-background p-4">
                        <div className="flex items-center justify-between text-sm">
                          <span className="font-semibold text-muted-foreground">
                            {filteredFiles.length} {t('research.files', { defaultValue: 'files' })}
                          </span>
                          <span className="font-bold text-primary">
                            {formatFileSize(research.files.reduce((acc, f) => acc + f.size_bytes, 0))}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="border-b-2 border-primary/10 bg-gradient-to-r from-background to-primary/5 p-5">
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3 min-w-0 flex-1">
                            {(() => {
                              const file = research.files[selectedFileIndex];
                              const FileIcon = getFileIcon(file?.mime_type || '', file?.name || '');
                              return (
                                <>
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-xs text-muted-foreground font-medium mt-0.5">
                                      {formatFileSize(file?.size_bytes || 0)} • {file?.mime_type}
                                    </p>
                                  </div>
                                </>
                              );
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => setSelectedFileIndex(prev => 
                                prev === 0 ? research.files.length - 1 : prev - 1
                              )}
                            ...
                            <button
                              onClick={() => setSelectedFileIndex(prev => 
                                prev === research.files.length - 1 ? 0 : prev + 1
                              )}
                              disabled={research.files.length <= 1}
                              className="p-2.5 rounded-xl border-2 border-primary/20 hover:bg-primary/5 hover:border-primary disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                            >
                      ...
                      <div className="flex-1 overflow-hidden relative">
                        <AnimatePresence mode="wait">
                          ...
                          >
                            {docs.length > 0 && (
                              <DocViewer
                                documents={[docs[selectedFileIndex]]}
                      ...
                      <div className="border-t-2 border-primary/10 bg-gradient-to-r from-background to-primary/5 p-5">
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            <button className="inline-flex items-center gap-2 rounded-xl border-2 border-primary/20 px-5 py-2.5 text-sm font-semibold hover:bg-primary/5 hover:border-primary transition-all">
                            ...
                            <a href={research.files[selectedFileIndex]?.url}
                            download
                          ...
                          >
                            ...
                            {t('actions.download', { defaultValue: 'Download File' })}
                          </a>
                        </div>
                      </div>
                    </div>
                ...
                >
                  ...
                  <p className="text-2xl font-bold mb-2">
                    {t('research.noFiles', { defaultValue: 'No Files Available' })}
                  </p>
                  <p className="text-muted-foreground max-w-md mx-auto">
                    {t('research.noFilesDesc', { defaultValue: 'Files and documents will be available once uploaded by the author.' })}
                  </p>
                </motion.div>
              )}
            </motion.div>
